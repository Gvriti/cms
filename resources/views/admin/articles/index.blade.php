                ...
                <div id="items">
                    ...
                    @foreach ($items as $item)
                        <li id="item{{ $item->id }}" class="item" data-id="{{ $item->id }}" data-pos="{{$item->position}}">
                            ...
                            @endif
                                <div class="list-label"><a href="{{ cms_route('articles.edit', [$parent->id, $item->id]) }}">{{ $item->title }}</a></div>
                                <div class="btn-action togglable pull-right">
                                    ...
                                    {!! Form::close() !!}
                                    <a href="{{ cms_route('files.index', ['articles', $item->id]) }}" class="btn btn-{{$item->files_cnt ? 'turquoise' : 'white'}}" title="{{trans('general.files')}}">
                                        <span class="{{icon_type('files')}}"></span>
                                    </a>
                                    <a href="{{ cms_route('articles.edit', [$parent->id, $item->id]) }}" class="btn btn-orange" title="{{trans('general.edit')}}">
                                        <span class="fa fa-edit"></span>
                                    </a>
                                    {!! Form::open(['method' => 'delete', 'url' => cms_route('articles.destroy', [$parent->id, $item->id]), 'class' => 'form-delete', 'data-id' => $item->id]) !!}
...
@push('scripts.bottom')
@include('admin._scripts.move', ['route' => cms_route('articles.move', [$parent->id]), 'column' => 'collection_id', 'list' => $similarTypes, 'parentId' => $parent->id])
