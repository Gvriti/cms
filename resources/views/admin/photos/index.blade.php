    ...
    <div class="row">
        <div class="col-sm-9 gallery-right">
            ...
            @foreach($items as $item)
                <li id="item{{$item->id}}" data-id="{{$item->id}}" data-pos="{{$item->position}}" data-url="{{cms_route('photos.edit', [$id, $item->id])}}" class="item col-lg-3 col-md-4 col-sm-4 col-xs-6">
...
$(document).ready(function($) {
    ...
    galleryEnv.on('click', 'a[data-modal]', function(e) {
        ...
        if (action == 'edit') {
            ...
            $.get("{!!cms_route('photos.create', [$id, 'sort' => $admin_sort, 'page' => $items->currentPage(), 'lastPage' => $items->lastPage()])!!}", function(data) {
                galleryEnv.append(data.view);
    ...
    galleryEnv.on('click', 'a[data-delete]', function(e) {
        ...
        if (ids.length) {
            ...
            $.post("{{cms_route('photos.index', [$id])}}/" + ids, input, function(data) {
...
@if ($admin_order_by == 'position')
    ...
    positionable('{{ cms_route('photos.updatePosition') }}', '{{$admin_sort}}', {{request('page', 1)}}, '{{$items->hasMorePages()}}');
@endif
});
</script>
