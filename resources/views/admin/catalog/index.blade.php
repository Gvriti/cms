                ...
                <div id="items">
                    ...
                    @foreach ($items as $item)
                        <li id="item{{ $item->id }}" class="item" data-id="{{ $item->id }}" data-pos="{{$item->position}}">
                            ...
                            @endif
                                <div class="list-label"><a href="{{ cms_route('catalog.edit', [$parent->id, $item->id]) }}">{{ $item->title }}</a></div>
...
@push('scripts.bottom')
@include('admin._scripts.move', ['route' => cms_route('catalog.move', [$parent->id]), 'column' => 'collection_id', 'list' => $similarTypes, 'parentId' => $parent->id])
