                ...
                <li id="item{{ $item->id }}" class="item{{$item->collapse ? ' uk-collapsed' : ''}}" data-id="{{ $item->id }}" data-pos="{{ $item->position }}">
                    <div class="uk-nestable-item clearfix">
                        ...
                        <div class="btn-action togglable pull-right">
                            ...
                            <a href="{{$cmsUrl = $item->collection_type ? cms_route($item->collection_type . '.index', [$item->type_id], null, true, false) : cms_route($item->type . '.'.($item->type_id ? 'edit' : 'index'), array_filter([$item->type_id]), [$item->type_id], null, true, false)}}" class="btn btn-{{$cmsUrl == '/#not_found' ? 'white disabled' : 'info'}}" title="{{$item->collection_title ?: ucfirst($item->type)}}">
