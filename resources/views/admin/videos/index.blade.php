...
<div class="page-title">
    <div class="title-env">
        <h1 class="title">
            <i class="{{$iconVideos = icon_type('videos')}}"></i>
            {{ $collection->title }}
        </h1>
    </div>
    ...
    <div class="row">
        <div class="col-sm-9 gallery-right">
            ...
            @foreach($items as $item)
                <li id="item{{$item->id}}" data-id="{{$item->id}}" data-pos="{{$item->position}}" data-url="{{cms_route('videos.edit', [$collection->id, $item->id])}}" class="item col-lg-4 col-md-6 col-sm-6 col-xs-12">
...
$(function() {
    var routeCreate = '{!!cms_route('videos.create', [$collection->id, 'sort' => $collection->admin_sort, 'page' => $items->currentPage(), 'lastPage' => $items->lastPage()])!!}';
    var routeIndex = '{{cms_route('videos.index', [$collection->id])}}';
    var routePosition = '{{cms_route('videos.updatePosition')}}';
    var sort = '{{$collection->admin_sort}}';
    var page = {{request('page', 1)}};
    var hasMorePages = '{{$items->hasMorePages()}}';
    @include('admin.scripts.album', ['orderBy' => $collection->admin_order_by, 'sort' => $collection->admin_sort])
});
</script>
